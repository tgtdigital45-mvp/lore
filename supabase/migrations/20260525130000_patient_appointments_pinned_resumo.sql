-- Fixar compromissos no cartão «Próximo agendamento» do Resumo (alinhado a medicamentos.pinned).

ALTER TABLE public.patient_appointments
  ADD COLUMN IF NOT EXISTS pinned boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.patient_appointments.pinned IS
  'Quando verdadeiro, o compromisso futuro mais próximo entre os fixados aparece no Resumo.';
...
COMMENT ON FUNCTION public.rpc_mobile_home_summary(uuid) IS
  'Bundle JSON do ecrã Resumo (mobile); próximo agendamento = compromissos com pinned=true.';
