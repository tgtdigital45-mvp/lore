...
export async function runOpenAiOcrVision(
  ...
  const userText = hint
    ? `O sistema já indicou o tipo clínico: ${hint}. Se document_suitability for "clinical_exam", o campo document_kind na resposta JSON deve ser exatamente "${hint}". markers_json como string JSON de objeto; metrics_json como string JSON de array. Responda apenas um objeto JSON com: summary_pt_br, exam_date_iso, title_pt_br, doctor_name, markers_json, metrics_json, confidence_note, document_suitability, document_kind, ui_category.`
    : `Defina document_suitability primeiro. markers_json e metrics_json como strings JSON. Responda apenas um objeto JSON com: summary_pt_br, exam_date_iso, title_pt_br, doctor_name, markers_json, metrics_json, confidence_note, document_suitability, document_kind, ui_category.`;
