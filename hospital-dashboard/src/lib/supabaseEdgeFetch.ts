 ...
 * Preferimos `NEXT_PUBLIC_SUPABASE_URL` no browser (igual ao cliente Supabase), para não depender
 * de proxy. Em dev, se a URL estiver em falta, usa caminho relativo (rewrites no `next.config`).
 */
...
/** POST JSON com JWT do usuário (mesmos cabeçalhos que `supabase.functions.invoke`). */
export async function postEdgeFunctionJson<T>(functionName: string, body: unknown): Promise<T> {
  ...
  if (!anon) {
    throw new Error("Configuração em falta (NEXT_PUBLIC_SUPABASE_ANON_KEY).");
  }
