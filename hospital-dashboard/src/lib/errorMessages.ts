...
const SUPABASE_ERROR_MAP: Record<string, string> = {
  ...
  "08006": "Conexão ao banco de dados interrompida.",
};
...
const MESSAGE_PATTERNS: Pattern[] = [
  ...
  { match: /user (is )?banned/i, pt: "Conta suspensa. Entre em contato com o suporte." },
  { match: /new row violates row-level security/i, pt: "Sem permissão para esta operação." },
  { match: /permission denied/i, pt: "Sem permissão para acessar esses dados." },
  { match: /violates foreign key constraint/i, pt: "Referência inválida (registro relacionado inexistente)." },
  ...
  { match: /could not find the table/i, pt: "Recurso não encontrado no banco de dados." },
  ...
  { match: /edge function returned a non-2xx status code/i, pt: "O serviço de relatório falhou. Tente novamente ou entre em contato com o suporte." },
  { match: /failed to fetch/i, pt: "Falha de rede. Verifique a conexão com a internet." },
  { match: /networkerror/i, pt: "Falha de rede. Verifique a conexão." },
  { match: /load failed/i, pt: "Falha ao carregar dados." },
  { match: /signal is aborted/i, pt: "Pedido cancelado ou tempo esgotado." },
  { match: /the user might not exist/i, pt: "Usuário não encontrado." },
  ...
  { match: /signup (is )?disabled/i, pt: "Novos registros estão desativados." },
...
export function userFacingApiError(err: unknown, contextFallback: string): string {
  const generic =
    "Ocorreu um erro ao processar os dados. Entre em contato com o suporte se persistir.";
