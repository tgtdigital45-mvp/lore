  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        aria-busy={loading}
        aria-label={loading ? "Carregando equipe" : "Lista da equipe"}
      >
        {loading ? (
          <>
            {[...Array(6)].map((_, i) => (
      ...
      {!loading && rows.length === 0 ? (
        <p className="text-center text-sm text-muted-foreground">Nenhum membro na equipe deste hospital.</p>
      ) : null}
...
function AddStaffModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!trimmed) {
      toast.error("Indique o e-mail do usuário.");
      return;
    }
    setBusy(true);
    try {
      ...
      if (!staffId) {
        toast.error("E-mail não encontrado. O usuário precisa se registrar primeiro.");
        return;
      }
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">E-mail do usuário já registrado no sistema.</p>
              </div>
              <button
                type="button"
          ...
          >
            ...
            <form onSubmit={(ev) => void submit(ev)} className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
              <div className="space-y-4 pb-24">
                ...
                <div>
                  <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">E-mail de contato</label>
                  <Input
                    type="email"
                    value={emailDisplay}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-slate-100 bg-slate-50/50 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-[0.65rem] text-muted-foreground">Máx. 2 MB. A foto fica visível na equipe.</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {open && row && pr ? (
        ...
        >
          <motion.div
            ...
            onClick={(ev) => ev.stopPropagation()}
          >
            <h2 className="text-lg font-bold text-slate-900">Remover da equipe?</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              <span className="font-semibold text-slate-800">{pr.full_name ?? pr.id}</span> deixará de ter acesso a este hospital. O usuário não é excluído.
            </p>
