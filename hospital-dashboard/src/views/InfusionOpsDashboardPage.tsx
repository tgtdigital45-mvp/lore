...
export function InfusionOpsDashboardPage() {
  ...
  return (
    <div className="relative min-h-full overflow-hidden pb-12">
      ...
      >
        {initialLoading ? <span className="sr-only">Carregando painel de infusão…</span> : null}
        <header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          >
            <Link href="/tv/operacao-infusao" target="_blank" rel="noopener noreferrer">
        ...
        <section aria-labelledby="postos-heading">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Cada cartão mostra o estado, a sessão em curso e o próximo paciente agendado.</p>
            </div>
          </div>
          {initialLoading ? (
            ...
            <p className="rounded-2xl border border-destructive/30 bg-destructive/5 px-4 py-6 text-center text-sm font-medium text-destructive" role="alert">
              {error}
            </p>
          ) : resources.length === 0 ? (
            <div
              className="rounded-3xl border border-dashed border-muted-foreground/35 bg-muted/20 px-6 py-14 text-center shadow-inner"
              role="status"
            >
              <p className="text-lg font-bold text-foreground">Nenhum posto configurado</p>
              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Ainda não existem cadeiras ou macas nesta unidade. Configure as posições na agenda hospitalar para ver o painel operacional.
              </p>
              <Button asChild className="mt-6 rounded-2xl" variant="secondary">
