 ...
 */
export function TriageWorkspaceLayout({ children }: { children: ReactNode }) {
  const { busy, loadError, rows, pendingLinkRequests, hospitalsMeta } = useOncoCare();
  ...
  const selectedPatientId = typeof params?.patientId === "string" ? params.patientId : undefined;
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2" role="group" aria-label="Filtro da fila de triagem">
              <Button
                type="button"
                variant="ghost"
                size="sm"
                aria-pressed={triageMode === "all"}
                className={cn(
              ...
              <Button
                ...
                variant="ghost"
                size="sm"
                aria-pressed={triageMode === "critical"}
                className={cn(
                  "rounded-full border px-3 py-1 text-xs font-semibold transition-all duration-200",
                  triageMode === "critical"
                    ? "border-lime-500 bg-lime-200 text-lime-900 shadow-card"
